...
function g({
    ...
    try {
        const t = new URL(e);
        return window.location.origin === t.origin && window.location.pathname !== t.pathname && !u.has(e)
    } catch {}
    return !1
}
let s, c;
...
function b(e) {
    u.add(e.href), c.observe(e), f.map(t => e.addEventListener(t, l, {
        passive: !0,
        once: !0
    }))
}
...
function y({
    ...
    if ("connection" in navigator) {
        ...
        if (/(2|3)g/.test(o.effectiveType)) return Promise.reject(new Error("Cannot prefetch, network conditions are poor"))
    }
    const [r, a] = m(t);
    c = c || new IntersectionObserver(o => {
        o.forEach(n => {
            n.isIntersecting && n.target instanceof HTMLAnchorElement && r(() => h(n.target).finally(a))
        })
    }), p(() => {
